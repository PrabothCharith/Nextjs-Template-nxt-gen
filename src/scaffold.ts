...
import ora from 'ora';
import { prismaSchema, prismaClient, axiosClient, queryProvider, providersComponent } from './templates/base';
import { exampleApiHandler, examplePage } from './templates/examples';

export const scaffoldProject = async (projectName: string, config: ProjectConfig) => {
...
export default function Home() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-between p-24">
      <h1 className="text-4xl font-bold">Welcome to ${path.basename(projectPath)}</h1>
    </div>
  );
}
`);
...
async function setupReactQuery(projectPath: string) {
    ...
    await fs.writeFile(path.join(projectPath, 'src/components/providers/query-provider.tsx'), queryProvider);
...
async function setupUI(projectPath: string, ui: string) {
    ...
    if (ui === 'shadcn' || ui === 'both') {
       await runCommand('npm', ['install', 'class-variance-authority', 'clsx', 'tailwind-merge', 'lucide-react'], projectPath);
       
       await fs.ensureDir(path.join(projectPath, 'src/lib'));
       await fs.writeFile(path.join(projectPath, 'src/lib/utils.ts'), `
...
`);
            ...
            if (!tailwindConfig.includes('@heroui/react')) {
                 tailwindConfig = "import {heroui} from '@heroui/react';\n" + tailwindConfig;
            }
            
            // Add plugin
            if (tailwindConfig.includes('plugins: [')) {
                if (!tailwindConfig.includes('heroui()')) {
                    tailwindConfig = tailwindConfig.replace('plugins: [', 'plugins: [heroui(),');
                }
            } else {
                 console.warn('Could not inject HeroUI plugin into tailwind.config.ts automatically. Please check manually.');
            }
...
async function setupExamples(projectPath: string, config: ProjectConfig) {
    ...
    if (config.examples === 'auth' || config.examples === 'both') {
         ...
         await fs.writeFile(path.join(projectPath, 'src/app/auth/page.tsx'), `
...
async function setupProviders(projectPath: string, config: ProjectConfig) {
    ...
    // Add import
    layoutContent = "import { Providers } from '@/components/providers';\n" + layoutContent;
